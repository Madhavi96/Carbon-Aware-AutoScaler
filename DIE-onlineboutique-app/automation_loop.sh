...
MODEL_CONFIG_PATH="./config/${APP_NAME}_dataset_speed.yaml"
...
MODEL_RESTRICTIONS=(0.35 0.2 0.5 0.35 0.2 0.5 0.35 0.2 0.5)  # Numeric restrictions
...
do
  ...
  do
    ...
    ssh -i train.pem ubuntu@$STGCN_IP << EOF
      ...
      # CHANGE THE SERVCES ARRAY IN predict_scale.py BEFORE RUNNING for each new application
      nohup python3 predict_scale.py --app_name=$APP_NAME --model_name=$MODEL_NAME --model_save_path=$MODEL_FILE --round=$ROUND --model_config_path=$MODEL_CONFIG_PATH --restriction=$MODEL_RESTRICTION > predict_T.log 2>&1 &
EOF
    ...
    ssh -i train.pem ubuntu@$LOCUST_IP << EOF
      ...
      bash load_test_${APP_NAME}.sh ${APP_NAME}/${MODEL_ID}/${MODEL_ID}_${MODEL_RESTRICTION}_${ROUND} $port
EOF
