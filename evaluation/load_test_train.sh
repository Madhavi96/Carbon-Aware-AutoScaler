...
# Start port forwarding in screen sessions
if ! screen -list | grep -q "\.port"; then
  screen -dmS port bash -c "ssh -i train.pem -L 32677:192.168.49.2:32677 ubuntu@195.148.22.200"
  echo "Started SSH tunnel in screen session 'port'"
else
  echo "Screen session 'port' already running."
fi
...
# TODO: CHECK ALWAYS IF NEEDED TO COMMENT OUT WHEN RUNNING EXPERIMENTS (INFERENCE)
locust -f load_generator_train.py --headless --csv "${output_prefix}/data"
locust -f load_generator_train.py --headless --csv "${output_prefix}/data_2" --only-summary
locust -f load_generator_train.py --headless --csv "${output_prefix}/data_3" --only-summary
locust -f load_generator_train.py --headless --csv "${output_prefix}/data_4" --only-summary
