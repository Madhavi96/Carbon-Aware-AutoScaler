...
if ! screen -list | grep -q "\.port1"; then
  screen -dmS port1 bash -c "ssh -i train.pem -L 30012:192.168.49.2:30012 ubuntu@195.148.22.200"
  echo "Started SSH tunnel in screen session 'port1'"
else
  echo "Screen session 'port1' already running."
fi
...
locust -f load_generator_bookinfo.py --headless --csv "${output_prefix}/data"
# sleep 720
# locust -f load_generator_bookinfo.py --headless --csv "${output_prefix}/data_2" --only-summary
# sleep 720
# locust -f load_generator_bookinfo.py --headless --csv "${output_prefix}/data_3" --only-summary
# sleep 720
# locust -f load_generator_bookinfo.py --headless --csv "${output_prefix}/data_4" --only-summary
# sleep 720
# locust -f load_generator_bookinfo.py --headless --csv "${output_prefix}/data_5" --only-summary
